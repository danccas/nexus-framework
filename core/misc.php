...
{
  return Core\Application::instance($container);
}
function kernel()
{
  return Core\Kernel::instance();
}
function db($cdr = null)
{
...
function humanSize($bytes)
{
  ...
  return $sign . round($bytes, 2, PHP_ROUND_HALF_UP) . ' ' . $units[$index];
}
...
function debug($x)
{
  ...
  echo "<pre>";
  print_r($x);
  $diff = (microtime(true) - DEBUG_START) * 1000;
  echo "<br /><br />Execute in: " . $diff . ' ms';
  exit;
}
...
/*Funciones de Popup y Tablefy */
function Popup_close($e = null)
{
  echo 'Popup-close';
  exit;
}
...
function Popup_ok($e = null)
{
  echo 'Popup-ok ' . $e;
  exit;
}
function Popup_location($e = null)
{
...
function get_tokenReq($length = 5)
{
  $token = "";
...
function data_to_tablefy($ls)
{
  ...
  $rp = array();
  foreach ($ls as $k => $n) {
    $n['Popup_is_father'] = false;
    $n['Popup_father_id'] = null;
    $rp[$n['id']] = $n;
  }
  foreach ($rp as $k => $n) {
    $rp[$k]['Popup_id'] = (int) $k;
    if (!empty($n['padre_id'])) {
      $rp[$k]['Popup_father_id'] = (int) $n['padre_id'];
      if (!empty($rp[$n['padre_id']])) {
        $rp[$rp[$k]['padre_id']]['Popup_is_father'] = true;
      }
    }
  }
...
function hora($x = null, $formato = 'h:i:s A')
{
