...
final class Route
{
  ...
  private function route($route, $regexps = false, $started = false, $delete = true)
  {
    ...
    if (!empty($e)) {
      ...
      $this->route['old'] = '/' . (!empty($this->route['tree']) ? implode('/', $this->route['tree']) : '');
  ...
  public static function setError($x)
  {
    ...
    if (static::requestByPopup()) {
      echo 'popup-error ';
      echo is_array($x) ? json_encode($x) : $x;
      exit;
    }
  ...
  public static function renderErrors()
  {
    ...
    if (!empty($ls)) {
      ...
      $html .= '<div class="rounded-t-lg overflow-hidden border-t border-l border-r border-gray-400 p-4">';
      foreach ($se as $type => $messages) {
        $html .= "<div class=\"bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative is-" . $type . "\">";
        $html .= "<strong class=\"font-bold\">Debes seguir estas indicaciones:</strong>";
        $html .= "<ul style=\"margin-top:0\">";
        foreach ($messages as $m) {
          $html .= "<span class=\"block sm:inline\">" . $m . "</span>";
        }
        $html .= "</ul>";
        $html .= "</div>";
      }
  ...
  static function href($a)
  {
    ...
    $h .= ($ce->data[$index][$a]['Popup'] ? ' data-Popup' : '');
    return $h;
  }
  ...
  static function requestByPopup($cb = null, $ncb = null)
  {
  ...
  static function renderNav()
  {
    ...
    if (!empty(static::data('submenu'))) {
      ...
      foreach (static::nav() as $nav) {
        //$h .= '<div class="column">';
        $h .= '<a href="' . $nav['link'] . '" ' . (!empty($nav['Popup']) ? 'data-Popup' : '') . ' class="button text-white bg-theme-1 shadow-md mr-2">' . $nav['nombre'] . '</a>';
        //$h .= '</div>';
      }
      //$h .= '</div>';
    }
  ...
  static function responseJSON($message = '', $code = 200)
  {
